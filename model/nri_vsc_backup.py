...
class RNNDecoder_NRI(MessagePassing):
    ...
    def forward(self, inputs, edge_index, z, pred_steps=1):
        ...
        pred_all = []
        hidden_node_init = nn.Parameter(torch.zeros(inputs.shape[0], self.msg_out_dim))
        ...
        for step in range(self.num_timesteps - 1):
            ...
            if step == 0:
                pred, hidden = self.propagate(edge_index, x=hidden_node_init, z=z,
                                              inputs=ins, hidden=hidden_node_init)
            else:
                pred, hidden = self.propagate(edge_index, x=hidden, z=z, inputs=ins, hidden=hidden)
