...
class VSCEncoder_NRI(MessagePassing2):
    ...
    def __init__(self, n_in, n_hid, n_pinputs, n_nodes, device, skip=True):
        ...
        self.pseudo_inputs = nn.Parameter(torch.empty(n_pinputs*n_nodes, n_in))
        self.edge_idx_p = torch.empty(n_nodes*n_pinputs, n_nodes)
        self.n_nodes = n_nodes
        self.n_in = n_in # [dim_in_feature * tsteps]
        self.n_edges = n_nodes * (n_nodes-1)
        self.selection = nn.Linear(n_nodes * n_in, n_pinputs)

        self.init_weights()
        self.init_edge_idx(n_pinputs, device)
    ...
    def init_weights(self):
        for m in self.modules():
            if isinstance(m, nn.Linear):
                nn.init.xavier_normal_(m.weight.data)
                m.bias.data.fill_(0.1)
        nn.init.xavier_normal_(self.pseudo_inputs)

    def init_edge_idx(self, n_pinputs, device):
        fully_connected = np.ones((self.n_nodes, self.n_nodes)) - np.eye(self.n_nodes)
        encoder_edge = np.where(fully_connected)
        encoder_edge = np.array([encoder_edge[0], encoder_edge[1]], dtype=np.int64)
        edgep_idx = np.zeros((2, n_pinputs*self.n_edges))
        for i in range(n_pinputs):
            edgep_idx[: ,self.n_edges*i:self.n_edges*(i+1)] = encoder_edge + i*self.n_nodes
        edgep_idx = torch.LongTensor(edgep_idx)
        self.edge_idx_p = edgep_idx.to(device)


    def forward(self, inputs, edge_index):
        # Eq 5
        x = self.mlp_eq5_embedding(inputs) # [sims * nodes, hid_dim]
        x_u = self.mlp_eq5_embedding(self.pseudo_inputs) # [#pinputs * nodes, hid_dim]
        ...
        x_u, x_edge_p = self.propagate(self.edge_idx_p, x=x_u, skip=not self.skip)

        x_sel = inputs.reshape(-1, self.n_nodes * self.n_in) #[batch * n_nodes , n_in] > [batch, n_nodes * n_in]
        u = self.selection(x_sel) # [sims * nodes, n_in] > [sims , n_pinputs]
        u_star = F.softmax(u) # can be replaced with Softmax

        _, x = self.propagate(edge_index, x=x, skip=self.skip, x_skip=x_edge) #[batch_size * edges, hid_dim]
        _, x_u = self.propagate(self.edge_idx_p, x=x_u, skip=self.skip, x_skip=x_edge_p)

        mu_sig = self.fc_out(x) #[batch_size * edges, edge_features]
        log_spike = F.relu(self.fc_log_spike(x))
        log_spike = -F.relu(log_spike)

        mu_sig_p = self.fc_out(x_u) #[batch_size * edges, edge_features]
        log_spike_p = F.relu(self.fc_log_spike(x_u))
        log_spike_p = -F.relu(log_spike_p)

        return mu_sig, log_spike, mu_sig_p, log_spike_p, u_star
